        ...
        messages: [
          {
            ...
            content: {
              ...
              text: [
                ...
                "1. list_entries / get_entry for overview; get_entry_field for the string you will edit.",
                "2. Send expected_field_hash from get_entry_field on every mutate.",
                "3. Prefer patch_json_field when the current value is JSON; otherwise str_replace_field. write_field only for full rewrites.",
                "4. After any string mutate, get_entry_field again before claiming success. Tool ok is not the user goal.",
                "5. On str_replace_field miss: do not retry guessed anchors. Re-read; if JSON use patch_json_field; else stop.",
                "6. Do not invent field apiIds — use get_content_type.",
                "7. After edits, publish_entry if the change should be public.",
                "8. Verify with get_published_entry when public read is enabled.",
              ].join("\n"),
            },
          },
        ],
